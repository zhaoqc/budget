	...
	<head>
		...
		<script language="javascript">
			...
			jQuery(document).ready(function(){
				...
				jQuery('#deputyDeptLeader').val('${deputyDeptLeader}');
				jQuery('#deptLeader').val('${deptLeader}');
				jQuery('#taxRate').val('${dailyForm.taxRate}');
				...
				jQuery("#testPop5").click(function() {
					var resultVal = window.showModalDialog('/budget/common/comm/OrgOneSelectTree.jsp?orgNo='+'<%=orgNo%>&selectType=checkbox&selectAll=true&type=zhl','500','400');
					if (undefined != resultVal) {
						jQuery('#htDept').val('');  
						jQuery('#tempht').val('');
						var varArr = resultVal.split("~~");
	...
	<div id="ld" style="width:100%;display:none">
		<form action="/costProj.do?method=save" method="post"
			...
			<table class="commonTable">
				<thead>
					<tr>
						<c:if test="${dprojTypeDetail=='this'}">	
							<td colspan="100%">本年项目申报</td>
						</c:if>	
						<c:if test="${dprojTypeDetail=='cross'}">	
			...
			<table class="commonTable">
				...
				<tr>
					...
					<td colspan="5">
						<textarea style="font-size: 16px;leading:50;width: 80%;line-height: 1.0;" name="dprojAim" rows="2" onchange="isChangeMethod(this)">${dailyForm.dprojAim}</textarea>
						<br>
					</td>
				</tr>
				<tr>
				...
				<tr>
					<td>
						<input  type="text" name="ytdTotalAmount" style="text-align:right"
							id="ytdTotalAmount" value="<fmt:formatNumber pattern='#,##0.00' value='${dailyForm.ytdTotalAmount}'/>" onchange="isChangeMethod(this)"/>
						元
						<br>
					</td>
					<td>
